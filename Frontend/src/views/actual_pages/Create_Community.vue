        ...
        const saveCommunity = () => {
            ...
            if (community.value.name && community.value.name.trim() && community.value.description) {
                if (community.value.id) {
                    ...
                    community.value.id = createId();
                    communities.value.push(community.value);
                    subgroups.value.push(community.value); 
...
<template>
    ...
    <div class="card p-fluid">
        ...
        <Textarea style="margin-top: 7px;"id="desc" placeholder="Your Message" :autoResize="true" rows="3" cols="100" />
        </div>
x
        <div class="flex">
        ...
        <div class="card">
            <h5>Add Members</h5>
            <MultiSelect v-model="multiselectValue" :options="multiselectValues" optionLabel="name" placeholder="Select Members" :filter="true" class="w-full">
        ...
        <Button label="Delete" icon="pi pi-trash" severity="danger" @click="confirmDeleteSelected" :disabled="!selectedCommunities || !selectedCommunities.length" />
        <div class="card">
    <h5>Existing Subgroups</h5>
    <DataTable :value="subgroups">
        ...
        <Column>
            <template #body="slotProps">
                <Button icon="pi pi-pencil" class="mr-2" severity="success" rounded @click="editCommunity(slotProps.data)" />
            </template>
        </Column>
    </DataTable>
</div>
        <Dialog v-model:visible="communityDialog" :style="{ width: '450px' }" header="Subgroup Details" :modal="true" class="p-fluid">
            <div class="field">
                <label for="name">Subgroup Name</label>
                <InputText id="name" v-model.trim="community.name" required="true" autofocus :invalid="submitted && !community.name" />
            ...
            <div class="field">
                ...
                <MultiSelect v-model="multiselectValue2" :options="multiselectValues" optionLabel="name" placeholder="Select Countries" :filter="true" class="w-full">
                    <template #value="slotProps">
                        <div class="inline-flex align-items-center py-1 px-2 bg-primary text-primary border-round mr-2" v-for="option of slotProps.value" :key="option.code">
                            <span :class="'mr-2 flag flag-' + option.code.toLowerCase()" style="width: 18px; height: 12px" />
                            <div>{{ option.name }}</div>
