...
def create_connection(max_retries=12, retry_interval=5):
    ...
    while retries < max_retries:
        try:
            ...
            connection = pika.BlockingConnection(pika.ConnectionParameters(
                host=rabbitmq_host, port=rabbitmq_port, heartbeat=3600, blocked_connection_timeout=3600))
    ...
    if connection is None:
        raise Exception(
            "logServer: Unable to establish a connection to RabbitMQ after multiple attempts.")
...
def callback(ch, method, properties, body):
    ...
    api_response = call_rest_api(log_data)

    # Check if the API response indicates success
    if api_response and api_response.get("Result", {}).get("Success", False):
        # Acknowledge the message if the API call was successful
        ch.basic_ack(delivery_tag=method.delivery_tag)
    else:
        # Do not acknowledge the message if the API call failed
        # The message will be redelivered after a while
        print("API call failed. Message not acknowledged.")
